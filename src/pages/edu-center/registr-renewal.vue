  ...
  <div class="main">
    ...
    <div v-if="current == 0" class="current0">
          ...
          <div v-for="(_, index) in formData" :key="index" class="container-box mb-4">
            <div class="container-box-top">
              <div class="flex flex-items-center">
                <span class="font-600 text-#222 text-4">{{ _.courseName }}</span>
                <span class="ml-4">
                ...
                <a-space :size="24">
                  ...
                  <a-form-item label="有效期至">
                    ...
                    <a-date-picker class="w-35" v-if="handleOver && _.expiryDate" :disabled="handleOver"
                      v-model:value="_.expiryDate" />
                    <div v-if="handleOver && _.expiryDate == ''" class="w-35">不限制</div>
                  </a-form-item>
                  <!-- 单课优惠 -->
      ...
      <a-affix :offset-bottom="bottom">
        <div class="step h-20 flex flex-center justify-end pr-10  footer bg-white rounded-4">
          <div class="text-5.6 totalPrice mr-12">应收金额：<span>¥ 2000.00</span></div>
          ...
          <div class="modal-left">
            <a-list>
              <a-list-item v-for="(item, index) in selectItems" :key="index" :class="{ active: activeIndex === index }"
                ...
                <span class="pl-2.5" v-else>{{ item.title }}</span> <span class="num">1</span>
              </a-list-item>
            </a-list>
          </div>
              ...
              <a-list>
                <a-list-item @click="changeSelectCourse(index)" :class="activeCourse == index ? 'activeCourse' : ''"
                  ...
                  <div class="m-r-b-l pt-1 pb-1">
                    <div class="text-4 text-#222 font-500 mb-1">{{ item.name }}</div>
                    <div>
        ...
        <div class="radio-pay-type">
          ...
          <div class="payList mt-4">
            <div class="payList-title">
              <span>*</span>收款方式 <span class="payList-tip ml-2">请选择</span>
            </div>
            <div class="pay">
              <a-checkbox-group v-model:value="pay">
                <label class="pay-box" :class="{ 'active': pay.includes(item.id) }"
                  ...
                  <div class="container-box-bottom">
                    <a-form layout="vertical" :model="item" ref="accountFormRefs">
                      <a-space :size="24">
                        ...
                        <!-- 支付日期 -->
                        <a-form-item label="支付日期" name="payDate"
                          :rules="[{ required: true, message: '请选择支付日期', trigger: 'change' }]">
        ...
        <div class="mt-3 fixedcss">
          <div class=" h-20 flex flex-center justify-end pr-10  bg-white rounded-4">
            <div class="text-5 totalPrice mr-12 flex">
              ...
              <div class="text-3.5 text-#666 font-400"> {{ paymentSummary }}</div>
            </div>
    ...
    <div v-if="current == 2" class="current2">
      <div class="step  bg-white rounded-4 mt-3 justify-start p-6">
        <div class="unfollowContainer flex flex-items-center">
          ...
          <div class="unfollowButtonContainer">点击邀请关注，推送家校通知 <RightOutlined /> </div>
        </div>
        <div class="patStatus">
          <div class="icon">
            <svg width="72px" height="72px" viewBox="0 0 72 72"><title>形状结合</title><g id="\u9875\u9762-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><g id="PC\u62A5\u540D\u7EED\u8D39" transform="translate(-694.000000, -288.000000)" fill="#FF9900"><path d="M730,288 C749.882251,288 766,304.117749 766,324 C766,343.882251 749.882251,360 730,360 C710.117749,360 694,343.882251 694,324 C694,304.117749 710.117749,288 730,288 Z M725.982064,302.866071 C724.206907,302.866071 722.767857,304.305132 722.767857,306.080302 L722.767857,306.080302 L722.767857,326.698561 L722.768382,326.772524 C722.803014,329.20975 724.546618,331.292687 726.947622,331.750368 L726.947622,331.750368 L742.897637,334.790775 L742.94993,334.800305 C744.674923,335.100229 746.327776,333.961588 746.656847,332.235253 L746.656847,332.235253 L746.666377,332.18296 C746.966299,330.457955 745.827666,328.805089 744.101344,328.476016 L744.101344,328.476016 L729.196271,325.634757 L729.196271,306.080302 L729.19584,306.027149 C729.167456,304.276494 727.73947,302.866071 725.982064,302.866071 Z" id="\u5F62\u72B6\u7ED3\u5408"></path></g></g></svg>
          </div>
          <div class="payPrice">
            ¥54.00
          </div>
...
import { ExclamationCircleFilled, RightOutlined,LeftOutlined } from '@ant-design/icons-vue';
...
watch(() => [...pay.value], (newVal, oldVal) => {
  ...
  removed.forEach(id => {
    const index = accountData.findIndex(item =>
...
.step {
  box-shadow: 0 0 8px 0 rgba(94, 188, 255, .08);
}
