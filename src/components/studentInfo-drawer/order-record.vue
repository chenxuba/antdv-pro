<template>
  <div class="order-record p3 pr0">
    <div class="record bg-white rounded-3 p3">
      <div class="total mb-1.5">共{{ dataSource.length }}条订单</div>
      <a-table :sticky="true"  :dataSource="dataSource" :columns="allColumns" :pagination="dataSource.length > 10" style="color: #666;"
        size="small">
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'orderStatus'">
            <div class="flex-center justify-start"><span class="dot"></span><span>已完成</span></div>
          </template>
          ...
          <template v-if="column.key === 'orderTotalPrice'">
            <div class="text-center">
              <div class="text-#222 font-500">+3000.00</div>
