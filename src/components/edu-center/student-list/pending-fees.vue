        ...
        <div class="table-content mt-2">
          <a-table :dataSource="dataSource" :pagination="dataSource.length>10" :columns="filteredColumns" :row-selection="rowSelection"
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'readingCourses'">初级感统课</template>
              <template v-if="column.key === 'headTeacher'">刘思君</template>
              <template v-if="column.key === 'remainingNum'">10课时</template>
              <template v-if="column.key === 'expireTime'">2025-04-24</template>
            </template>
          </a-table>
        </div>
      </div>
