...
const selectQuickFilter = (selectedFilter, type) => {
  if (type == 1) {
    ...
    console.log(
      "当前快捷筛选:",
      quickFilters.value.find((q) => q.selected)?.name
    );
  } else if (type == 2) {
    ...
    console.log(
      "当前快捷筛选:",
      quickApproveFilters.value.find((q) => q.selected)?.name
    );
  }
};
  ...
  const conditions = [
    ...
    },
    {
      ...
      values: (() => {
        ...
        const item = createPeoOptions.value.find(
    ...
    },
    {
      ...
      values: (() => {
        ...
        const item = stuListOptions.value.find(
    ...
    },
    {
      ...
      values: (() => {
        ...
        const item = classOptions.value.find(
    ...
    },
    {
      ...
      values: createPeoOptions.value.filter(
        (opt) => opt.id === createPeoVals.value
      ),
    },
    {
      type: "createTime",
    ...
    },
    {
      type: "payDate",
      label: "支付日期",
      show: props.displayArray.includes("payDate"),
      values:
        createTimeVals.value.length === 2
          ? [
              {
    ...
    },
    {
      ...
      values: courseListOptions.value.filter(
    ...
    },
    {
      ...
      values: stuListOptions.value.filter(
        (opt) => opt.id === selectStuVals.value
      ),
    },
    {
    ...
    <div class="flex-1 mr-0">
      ...
      <div class="flex">
        <div class="filter-section mb-2 flex-1" v-if="isQuickShow">
          ...
          <div class="quick-filters">
            <a-button
              ...
              :key="filter.id"
              :type="filter.selected ? 'primary' : 'default'"
              class="filter-btn"
              @click="selectQuickFilter(filter, 1)"
            >
        ...
        <div class="filter-section mb-2 flex-1" v-if="isQuickOneToOneShow">
          ...
          <div class="quick-filters">
            <a-button
              v-for="filter in quickOneToOneFilters"
              :key="filter.id"
              :type="filter.selected ? 'primary' : 'default'"
              class="filter-btn"
              @click="selectQuickFilter(filter, 2)"
            >
              {{ filter.name }}（{{ filter.count }}）
            </a-button>
          </div>
        ...
        <div class="filter-section mb-2 flex-1" v-if="isApproveQuickShow">
          ...
          <div class="quick-filters">
            <a-button
              v-for="filter in quickApproveFilters"
              :key="filter.id"
              :type="filter.selected ? 'primary' : 'default'"
              class="filter-btn"
              @click="selectQuickFilter(filter, 3)"
            >
              {{ filter.name }}（{{ filter.count }}）
            </a-button>
          </div>
        ...
        <div class="w-100 mt--1" v-if="isShowSearchStuPhone">
          <div class="selectBox flex">
            ...
            <div>
              <a-select
                allowClear
                v-model:value="searchKeyStuPhone"
        ...
        <div class="w-100 mt--1" v-if="isShowOneToOne">
          <div class="selectBox flex">
            ...
            <div>
              <a-select
                allowClear
                v-model:value="searchKeyOneToOne"
                :filter-option="filterOption"
                show-search
                placeholder="请输入关键字"
                style="width: 240px"
      ...
      <div class="filter-section flex justify-between">
        <div class="flex">
          ...
          <div class="standard-filters">
            <checkbox-filter
              v-if="displayArray.includes('intention')"
              v-model:checkedValues="selectedValues"
              :options="customOptions"
              label="意向度"
              @change="handleIntentionChange"
              type="checkbox"
            />
            <checkbox-filter
              v-if="displayArray.includes('followStatus')"
              v-model:checkedValues="followStatusVals"
            ...
            />
            <checkbox-filter
              v-if="displayArray.includes('sex')"
              v-model:checkedValues="sexVals"
              :options="sexOptions"
              label="性别"
            ...
            />
            <checkbox-filter
              v-if="displayArray.includes('createPeo')"
              :ref="(el) => handleRef(el, 'createUser')"
              category="teacher"
              placeholder="请输入创建人"
              v-model:checkedValues="createPeoVals"
              :options="createPeoOptions"
              label="创建人"
              @radioChange="handleCreatePeoChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('createTime')"
              v-model:checkedValues="createTimeVals"
              label="创建时间"
              @datePickerChange="handleCreateTimeChange"
              type="dateTime"
            />
            <checkbox-filter
              v-if="displayArray.includes('applyTime')"
              v-model:checkedValues="createTimeVals"
              label="申请时间"
              @datePickerChange="handleCreateTimeChange"
              type="dateTime"
            />
            <checkbox-filter
              v-if="displayArray.includes('payDate')"
              v-model:checkedValues="createTimeVals"
              label="支付日期"
              @datePickerChange="handleCreateTimeChange"
              type="dateTime"
            />
            <checkbox-filter
              v-if="displayArray.includes('classEndingTime')"
              v-model:checkedValues="classEndingTimeVals"
              label="结课时间"
              @datePickerChange="handleCreateTimeChange"
              type="dateTimeQuick"
            />
            <checkbox-filter
              v-if="displayArray.includes('classStopTime')"
              v-model:checkedValues="classStopTimeVals"
              label="停课时间"
              @datePickerChange="handleCreateTimeChange"
              type="dateTimeQuick"
            />
            <checkbox-filter
              v-if="displayArray.includes('intentionCourse')"
              :ref="(el) => handleRef(el, 'yiXiangcourse')"
              category="course"
              placeholder="请输入意向课程"
              v-model:checkedValues="selectCourseValues"
              :options="courseListOptions"
              label="意向课程"
              @radioChange="handleCourseChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('reference')"
              :ref="(el) => handleRef(el, 'tuijianren')"
              category="stu"
              placeholder="请输入推荐人"
              v-model:checkedValues="selectStuVals"
              :options="stuListOptions"
              label="推荐人"
              @radioChange="handleReferenceChange"
              type="radio"
            />
            <checkbox-filter
              v-if="type == 'dpt' && displayArray.includes('department')"
              v-model:checkedValues="selectDptVals"
              :options="dptListOptions"
              label="所属部门"
              type="tree"
            />
            <checkbox-filter
              v-if="displayArray.includes('channelCategory')"
              showSearch
              v-model:checkedValues="selectChannelCategoryVals"
              :options="channelCategoryOptions"
              label="渠道分类"
              @change="handleChannelCategoryChange"
              type="checkbox"
            />
            <checkbox-filter
              v-if="displayArray.includes('channelStatus')"
              v-model:checkedValues="selectChannelVals"
            ...
            />
            <checkbox-filter
              v-if="displayArray.includes('channelType')"
              v-model:checkedValues="selectChannelTypeVals"
            ...
            />
            <checkbox-filter
              v-if="displayArray.includes('subject')"
              :ref="(el) => handleRef(el, 'kemu')"
              category="course"
              placeholder="请输入科目"
              v-model:checkedValues="selectSubjectVals"
              :options="subjectOptions"
              label="科目"
              @radioChange="handleSubjectChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('courseCategory')"
              :ref="(el) => handleRef(el, 'courseType')"
              category="course"
              placeholder="请选择课程类别"
              v-model:checkedValues="selectCourseCategoryVals"
              :options="courseCategoryOptions"
              label="课程类别"
              @radioChange="handleCourseCategoryChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('studentStatus')"
              :ref="(el) => handleRef(el, 'studentStatus')"
              category="noSearchRadio"
              placeholder="选择学员状态"
              v-model:checkedValues="selectStudentStatusVals"
              :options="studentStatusOptions"
              label="学员状态"
              @radioChange="handleStudentStatusChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('currentStatus')"
              :ref="(el) => handleRef(el, 'currentStatus')"
              category="course"
              placeholder="选择状态"
              v-model:checkedValues="selectCurrentStatusVals"
              :options="currentStatusOptions"
              label="当前状态"
              @change="handleCurrentStatusChange"
              type="checkbox"
            />
            <checkbox-filter
              v-if="displayArray.includes('orNotFenClass')"
              :ref="(el) => handleRef(el, 'orNotFenClass')"
              v-model:checkedValues="selectOrNotFenClassVals"
              :options="orNotFenClassOptions"
              label="是否分班"
              @change="handleOrNotFenClassChange"
              type="checkbox"
            />
            <checkbox-filter
              v-if="displayArray.includes('billingMode')"
              :ref="(el) => handleRef(el, 'billingMode')"
              v-model:checkedValues="selectBillingModeVals"
            ...
            />
            <checkbox-filter
              v-if="displayArray.includes('isSetExpirationDate')"
              :ref="(el) => handleRef(el, 'isSetExpirationDate')"
              category="noSearchRadio"
              placeholder="请选择有效期状态"
              v-model:checkedValues="selectIsSetExpirationDateVals"
              :options="isSetExpirationDateOptions"
              label="是否设置有效期"
              @radioChange="handleIsSetExpirationDateChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('openClassStatus')"
              :ref="(el) => handleRef(el, 'openClassStatus')"
              category="noSearchRadio"
              placeholder="请选择开班状态"
              v-model:checkedValues="selectOpenClassStatusVals"
              :options="openClassStatusOptions"
              label="开班状态"
              @radioChange="handleOpenClassStatusChange"
              type="radio"
            />
            <checkbox-filter
              v-if="displayArray.includes('doYouSchedule')"
              :ref="(el) => handleRef(el, 'doYouSchedule')"
              category="noSearchRadio"
              placeholder="请选择排课状态"
              v-model:checkedValues="selectDoYouScheduleVals"
              :options="doYouScheduleOptions"
              label="是否排课"
              @radioChange="handleDoYouScheduleChange"
              type="radio"
            />
          </div>
        </div>
        ...
        <div class="w-100 mt--0.5" v-if="isShowChangTeacherSearch">
          <div>
            <a-input-group compact class="compactInput">
              ...
              >
                <a-select-option
                  v-for="(item, index) in createPeoOptions"
                  :key="index"
                  :value="item.id"
                  :data="item"
      ...
      <div
        ...
        v-if="orderedConditions.length > 0 || type == 'dpt'"
      >
        <span class="section-title text-#222">已选条件：</span>
        <div class="condition-tags">
          <a-tag
            v-if="type != 'dpt' || orderedConditions.length > 0"
            color="red"
